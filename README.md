...
- Python 3.12 (`uv`가 자동으로 설치)

### Windows 배포판 사용자

- 요구사항 없음! `.exe` 파일을 바로 실행할 수 있습니다.
- [Windows 배포판 사용 방법](#windows-배포판-사용-방법-exe) 섹션을 참고하세요.
...
```bash
uv run uv run playwright install chromium
```
...
## Windows 배포판 사용 방법 (.exe)

동료들에게 전달할 Windows 실행 파일을 사용하는 방법입니다.

### 1. 압축 파일 받기

전달받은 `ShifteeAnalyzer.zip` 파일을 압축 해제합니다.

### 2. 실행

1. `ShifteeAnalyzer.exe`를 더블 클릭
2. GUI 로그인 창에서 정보 입력:
   - **아이디**: Shiftee 로그인 이메일
   - **비밀번호**: Shiftee 비밀번호
   - **팀 필터** (선택사항): 분석할 팀명 (쉼표로 구분, 예: `뱅킹IS팀,뱅킹통신보안팀`)
3. "로그인" 버튼 클릭

### 3. 결과 확인

프로그램 실행 후:
- `output/` 폴더에 분석 리포트 생성 (`report_YYYYMMDD_HHMMSS.xlsx`)
- `data/` 폴더에 다운로드한 원본 데이터 저장

### 주의사항

- 처음 실행 시 Windows Defender가 경고할 수 있습니다. "추가 정보" → "실행" 클릭
- 인터넷 연결이 필요합니다 (Shiftee 접속용)
- 로그인 정보는 암호화되어 로컬에 저장됩니다 (비밀번호 제외)

---

## 개발자용 사용법

설치가 완료되면 프로젝트 루트에서 `uv run shiftee-analyze` 명령으로 실행할 수 있습니다.
...
```bash
uv run shiftee-analyze --download --start 2025-11-01 --end 2025-11-30
```
...
```bash
# 위험 직원 목록 전송
uv run shiftee-analyze --send-kakao

# 다운로드 + 분석 + 전송
uv run shiftee-analyze --download --send-kakao
```
...
# 2. 기존 파일로 분석만 수행
uv run shiftee-analyze --start 2025-11-01 --end 2025-11-30
...
uv run shiftee-analyze --download --start 2025-12-01 --end 2025-12-15 --send-kakao
```
...
# 해결 방법 2: 파일 경로 직접 지정
uv run shiftee-analyze --data1 path/to/file1.xlsx --data2 path/to/file2.xlsx --start 2025-11-01 --end 2025-11-30
```
...
# 2. 브라우저 표시 모드로 디버깅
SHIFTEE_HEADLESS=false uv run shiftee-analyze --download
```
...
uv run python -c "import openpyxl; wb = openpyxl.load_workbook('data/shiftee_data1.xlsx'); print(wb.sheetnames)"
```
...
```bash
# 패키지 설치 확인
uv run python -c "import shiftee; print(shiftee.__file__)"
...
1. `dist/ShifteeAnalyzer/` 폴더 전체를 압축
2. 동료들에게 압축 파일 전달
3. 압축 해제 후 `ShifteeAnalyzer.exe` 실행

### 빌드 결과물

```
dist/ShifteeAnalyzer/
├── ShifteeAnalyzer.exe       # 메인 실행 파일
├── ms-playwright/             # Chromium 브라우저
├── _internal/                 # Python 라이브러리들
└── README.md                  # 사용 설명서
```

**참고**: 전체 폴더 크기는 약 200-300MB입니다 (Chromium 브라우저 포함).
...
- **💬 카카오톡 설정**: [`src/kakao_send/README.md`](src/kakao_send/README.md)
- **🪟 Windows 배포**: 위의 [Windows 배포판 빌드 방법](#windows-배포판-빌드-방법) 섹션 참고
